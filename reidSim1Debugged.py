...
VISUALIZER_RATES = [
    m / DT_SEC for m in (0.0625, 0.125, 0.25, 0.5, 1.0, 2.0, 4.0, 8.0)]
...
# Avoid distribution center
DELIVERY_SITE_X_BOUNDS = (100.0, WORLD_LENGTH - 100.0)
DELIVERY_SITE_Y_BOUNDS = (-WORLD_WIDTH_HALF + 5.0,
                          WORLD_WIDTH_HALF - 5.0)  # Avoid wrap-around
...
LIDAR_ANGLES = [(i - 15.0) * math.pi / 180 for i in range(0, 31)]
...
class Entity():
    ...
    def move(self, delta):
        v_x, v_y = delta
        self.position = (
            (self.position[0] + v_x) % WORLD_LENGTH, (self.position[1] + v_y) % WORLD_WIDTH)
...
class Camera(Entity):
    ...
    def project(self, position):
        ...
        projected_x = round(-((position[1] - camera_y) %
                            WORLD_WIDTH) * self._scale_inv + SCREEN_WIDTH_HALF)
    ...
    def draw(self, camera, surface):
        for projected_pos in camera.project(self.position):
            if self._fall_duration > 0:
                surface.blit(self._parachute_image,
                             (projected_pos[0] - 4, projected_pos[1] - 4))

            else:
                surface.blit(self._package_image,
                             (projected_pos[0] - 4, projected_pos[1] - 4))
...
class Zip(Circle):
    ...
    def draw(self, camera, surface):
        for projected_pos in camera.project(self.position):
            surface.blit(
                self._image, (projected_pos[0] - 16, projected_pos[1] - 16))
...
class DeliverySite(Circle):
    ...
    def draw(self, camera, surface):
        for projected_pos in camera.project(self.position):
            surface.blit(
                self._image, (projected_pos[0] - 64, projected_pos[1] - 64))
...
class Tree(Circle):
    ...
    def draw(self, camera, surface):
        for projected_pos in camera.project(self.position):
            surface.blit(
                self._image, (projected_pos[0] - 32, projected_pos[1] - 32))
...
class Wind():
    ...
    def update(self, dt):
        # TODO: Scale sigma?
        self._speed = max(0.0, min(MAX_WINDSPEED_M_S,
                          self._speed + random.gauss(0.0, dt * 10)))
        self._direction = (self._direction +
                           random.gauss(0.0, dt)) % (2 * math.pi)
...
def cast_lidar_ray(angle, circles):
    ...
    for o in circles:
        if o[0] * o[0] + o[1] * o[1] <= o[2] * o[2]:
            # We're inside the object. Pretend that the lidar is blind.
            return 0
...
def cast_lidar(start_pos, objects):
    ...
    relative_objects = [(o.position[0] - start_pos[0],
                         (o.position[1] - start_pos[1] +
                          WORLD_WIDTH_HALF) % WORLD_WIDTH - WORLD_WIDTH_HALF,
...
if __name__ == "__main__":
    ...
    parser.add_argument('pilot', nargs=argparse.REMAINDER,
                        help='A pilot process to run')
    parser.add_argument('--headless', action="store_true",
                        help='Run without visualization')
    visualizer_group = parser.add_argument_group("Visualization options")
    visualizer_group.add_argument(
        '--chase-y', action="store_true", help='Have the camera follow the zip in the y axis')
    visualizer_group.add_argument(
    ...
    while result is None:
        ...
        if api_mode:
            ...
            if len(cmd) != COMMAND_STRUCT.size:
                ...
                break
            lateral_airspeed_input, drop_package_commanded_byte, _ = COMMAND_STRUCT.unpack(
                cmd)
            ...
            if keys[pygame.K_RIGHT]:
                lateral_airspeed = max(-30.0,
                                       lateral_airspeed - DT_SEC * 200.0)
        ...
        if drop_package_commanded and not was_package_dropped and num_packages > 0:
            ...
            dropped_packages.append(
                Package(vehicle.position, vehicle.get_velocity(lateral_airspeed, wind.vector)))
        ...
        if not headless:
            ...
            camera.position = (
                vehicle.position[0] + CAMERA_AHEAD_M, vehicle.position[1] if chase_y else 0.0)

            terrain.draw(camera, screen)
            # Draw distribution center
            for pos in camera.project((0, 0)):
                screen.blit(distribution_center_image,
                            (pos[0] - 250, pos[1] - 100))
            ...
            reticle.move(
                (v * PACKAGE_FALL_SEC for v in vehicle.get_velocity(lateral_airspeed, wind.vector)))
                ...
                for e in pygame.event.get():
                    ...
                    if e.type in [pygame.KEYDOWN]:
                        ...
                        if e.key == pygame.K_COMMA:
                            visualizer_rate_index = max(
                                0, visualizer_rate_index - 1)
